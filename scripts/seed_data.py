...
def seed_data():
    customer_wb = load_workbook("./assets/customer_data.xlsx")
    customer_sheet = customer_wb.active

    loan_wb = load_workbook("./assets/loan_data.xlsx")
    ...
    for row in customer_sheet.iter_rows(min_row=2, values_only=True):
        ...
        if customer_id is not None:
            ...
            print(f"Created Customer {customer_id}: {first_name} {last_name}")
    ...
    for row in loan_sheet.iter_rows(min_row=2, values_only=True):
        ...
        if customer_id is not None and customer_id in customer_map:
            ...
            print(f"Created Loan {loan_id} for Customer {customer.id}")
        else:
    ...
    for customer in customers:
        ...
        customer.save()
        print(f"Updated Customer {customer.id} Current Debt to {customer.current_debt}")
